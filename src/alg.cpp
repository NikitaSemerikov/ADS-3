...
uint64_t collatzMaxValue(uint64_t num) {
  ...
  while (num != 1) {
    if (num % 2 == 0) {
      ...
      // проверка переполнения: 3*num+1 <= UINT64_MAX
      if (num > (UINT64_MAX - 1) / 3) {
        // переполнение невозможно корректно обработать — возвращаем максимум
        return UINT64_MAX;
      }
      num = 3 * num + 1;
    }
...
unsigned int collatzLen(uint64_t num) {
  ...
  while (num != 1) {
    if (num % 2 == 0) {
      ...
      if (num > (UINT64_MAX - 1) / 3) {
        // переполнение — длину корректно продолжить невозможно
        return len;
      }
      num = 3 * num + 1;
    }
